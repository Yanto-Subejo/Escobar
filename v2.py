...
sys.stdout.write('\x1b[1;35m\x1b]2;@Moch Aang Ardiansyah XD\x07')
sys.stdout.write('\n\n\x1b[1;92m                           #    Moch Aang Ardiansyah XD    #\n\n');jeda(4)
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		print ('\n%s%s%s 1 %sLogin instagram (crack akun instagram) '%(U,til,K,O))
		print ('%s%s%s 2 %sLogin via cookie (crack akun facebook) '%(U,til,K,O))
		print ('%s%s%s 3 %sCara mendapatkan cookie facebook '%(U,til,K,O))
		print ('%s%s%s 0 %sKeluar '%(U,til,M,O))
		while True:
			...
			if rom in(""):
				...
				kukis = input("%s# %sCookie %s> %s"%(P,O,M,H))
				if kukis in(""):
...
def konverter(kukis): 
	...
	try:
		...
		print("%s%s terjadi kesalahan saat convert, periksa cookie anda "%(M,til));exit()
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			banner()
			print(f"{U}#{O} My Team{H}  : {O}XNXCODE TEAM 2021\n")
			...
			print ('%s•%s 13 %sHapus data login'%(U,P,O))
			print ('%s•%s 00 %sGabung grup %sXNXCODE%s'%(U,P,O,H,P))
...
class pilihan:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			os.system("xdg-open https://chat.whatsapp.com/GAghwpRIBIqHDJLSITcFxD")
